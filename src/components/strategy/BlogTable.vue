  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="标题"
        align="left">
        <template slot-scope="scope"><span style="color: #409eff;cursor: pointer" @click="itemClick(scope.row)">{{ scope.row.strTitle}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="总结"
        width=200px;
        align="left">
        <template slot-scope="scope"><span style="color: #409eff;cursor: pointer" @click="itemClick(scope.row)">{{ scope.row.summary.substring(0,30)}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="公司"
        align="left">
        <template slot-scope="scope"><span>{{ scope.row.company}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="产品"
      ...
      <el-table-column
        label="最近编辑时间" width="80px" align="left">
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-if="articles&&articles.length>0 && showDelete"
                 :disabled="selItems.length==0" @click="deleteMany">批量删除
      </el-button>
      <span></span>
      <el-pagination
        background
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      itemClick(row){
        this.$router.push({path: '/strategyDetail', query: {aid: row.id}})
      },
      deleteMany(){
      ...
      loadBlogs(page, count){
        ...
        var url = '';
        if (this.state == -2) {
          url = "/admin/strategy/all" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        } else {
          url = "/strategy/all?state=" + this.state + "&page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        }
        getRequest(url).then(resp=> {
          console.log(resp)
          _this.loading = false;
          if (resp.status == 200) {
            _this.articles = resp.data.strategy;
      ...
      handleEdit(index, row) {
        this.$router.push({path: '/editStrategy', query: {from: this.activeName,id:row.id}});
      },
      ...
      deleteToDustBin(state){
        ...
        this.$confirm(state != 2 ? '将该文件放入回收站，是否继续?' : '永久删除该文件, 是否继续?', '提示', {
          ...
          var url = '';
          if (_this.state == -2) {
            url = "/admin/strategy/dustbin";
          } else {
            url = "/strategy/dustbin";
          }
