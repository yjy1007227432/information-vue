  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        label="标题"
        align="left">
        <template slot-scope="scope"><span style="color: #409eff;cursor: pointer" @click="itemClick(scope.row)">{{ scope.row.title.substring(0,30)}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="总结"
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      itemClick(row){
        this.$router.push({path: '/infoDetail', query: {aid: row.id}})
      },
      deleteMany(){
      ...
      loadBlogs(page, count){
        ...
        var url = '';
        if (this.state == -2) {
          url = "/admin/information/all" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        } else {
          url = "/information/all?state=" + this.state + "&page=" + page + "&count=" + count + "&keywords=" + this.keywords;
        }
        getRequest(url).then(resp=> {
          console.log(resp)
          _this.loading = false;
          if (resp.status == 200) {
            _this.articles = resp.data.informations;
      ...
      handleEdit(index, row) {
        this.$router.push({path: '/editInfo', query: {from: this.activeName,id:row.id}});
      },
      ...
      deleteToDustBin(state){
        ...
        this.$confirm(state != 2 ? '将该文件放入回收站，是否继续?' : '永久删除该文件, 是否继续?', '提示', {
          ...
          var url = '';
          if (_this.state == -2) {
            url = "/admin/information/dustbin";
          } else {
            url = "/information/dustbin";
          }
